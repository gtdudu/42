...
Bureaucrat::Bureaucrat( int grade ) : _name("bury") {
  if (grade < 1) {
...
void          Bureaucrat::signForm(std::string fn, int fg, bool ok) const {
  if (ok) {
    std::cout << this->_name << " signs " << fn << std::endl;
  } else {
    std::cout << this->_name << "(grade: " << this->_grade << ") cannot sign " << fn << " because form requires grade " << fg << " to be signed." << std::endl;
  }
};
